...
(func $applejag/firefly-jam-2026.boot (result i32)
 ...
 (i32.store offset=8
  ...
  (local.get $*tmp/76))
 (i32.const 0))
(func $@applejag/firefly-jam-2026/racebattle.World::update (param $self/24 i32) (result i32)
 (local $camera/75 i32)
 (local $*field/85 i32)
 (call $moonbit.incref
  (local.tee $camera/75
 ...
 (call $@applejag/firefly-jam-2026/racebattle.Camera::update
  (local.get $camera/75)
  (local.get $self/24)))
(func $@applejag/firefly-jam-2026/racebattle.Camera::update (param $self/23 i32) (param $world/20 i32) (result i32)
 ...
 (local $*tmp/0/70 f32)
 (local $*tmp/1/70 f32)
 (local $players_pos/71 i32)
 (local $player_system/72 i32)
 (local $players_peer/73 i32)
 ...
 (call $moonbit.incref
  (local.tee $players_peer/73
   (local.tee $*field/89
    (i32.load offset=8
     (local.tee $player_system/74
      (local.tee $*field/90
       (i32.load offset=8
        (local.get $world/20))))))))
 (if (result i32)
  ...
  (else
   ...
   (local.set $*field/87
    (i32.load offset=12
     (local.tee $player_system/72
      (local.get $*field/88))))
   (if
    (i32.gt_s
     (local.tee $*cnt/98
      (i32.load
       (local.get $player_system/72)))
     (i32.const 1))
    (then
     (local.set $*new_cnt/101
      (i32.sub
       (local.get $*cnt/98)
       (i32.const 1)))
     (i32.store
      (local.get $player_system/72)
      (local.get $*new_cnt/101))
     (call $moonbit.incref
      (local.get $*field/87)))
    (else
     (if
